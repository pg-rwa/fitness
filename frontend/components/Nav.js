...
const clientNav = [
  ...
  { href: "/dashboard/templates", label: "Templates", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" },
  { href: "/dashboard/workouts", label: "Workouts", icon: "M4 6h16M4 10h16M4 14h16M4 18h16" },
  { href: "/dashboard/nutrition", label: "Nutrition", icon: "M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z" },
  ...
  { href: "/dashboard/insights", label: "Insights", icon: "M13 10V3L4 14h7v7l9-11h-7z" },
  ...
  { href: "/dashboard/schedule", label: "Schedule", icon: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" },
  { href: "/dashboard/social", label: "Social", icon: "M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" },
  ...
  return (
    <>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-gray-900 border-t border-gray-800 z-50 md:hidden">
        {/* Overflow menu */}
        {moreOpen && (
          <div className="border-b border-gray-800 bg-gray-900 px-2 py-2 grid grid-cols-4 gap-1">
            {mobileOverflow.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`flex flex-col items-center gap-0.5 px-2 py-2 rounded-lg ${active ? "text-brand-500 bg-brand-500/10" : "text-gray-400"}`}
                >
        ...
        <div className="flex justify-around py-2">
          {mobileMain.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={() => setMoreOpen(false)}
                className={`flex flex-col items-center gap-0.5 px-2 py-1 ${active ? "text-brand-500" : "text-gray-500"}`}
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d={item.icon} />
                </svg>
          ...
          <button
            ...
            className={`flex flex-col items-center gap-0.5 px-2 py-1 ${moreOpen || isOverflowActive ? "text-brand-500" : "text-gray-500"}`}
          >
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      {/* Click-away overlay to close more menu */}
      {moreOpen && (
        <div className="fixed inset-0 z-40 md:hidden" onClick={() => setMoreOpen(false)} />
      )}

      {/* Desktop sidebar */}
      <aside className="hidden md:flex flex-col w-56 min-h-screen bg-gray-900 border-r border-gray-800 p-3 fixed left-0 top-0">
          ...
          {items.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${active ? "bg-brand-500/10 text-brand-500 font-medium" : "text-gray-400 hover:text-white hover:bg-gray-800"}`}
              >
        ...
        <div className="border-t border-gray-800 pt-3 mt-3">
          ...
          <button
            onClick={logout}
            className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-gray-400 hover:text-red-400 hover:bg-gray-800 transition-colors w-full"
          >
            <svg className="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
