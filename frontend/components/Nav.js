...
// Mobile: just 4 tabs — Dashboard, Workouts/Clients, Templates/Calendar, Profile
...
export default function Nav() {
  ...
  const mobileTabs = isTrainer ? trainerMobileTabs : clientMobileTabs;

  return (
    <>
      {/* Mobile bottom nav — clean 4 tabs, no More button */}
      <nav className="fixed bottom-0 left-0 right-0 bg-gray-900/95 backdrop-blur-lg border-t border-gray-800 z-50 md:hidden">
        <div className="flex justify-around py-2 px-2">
          {mobileTabs.map((item) => {
            ...
            return (
              ...
              >
                <svg className={`w-5 h-5 ${active ? "scale-110" : ""} transition-transform`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2 : 1.5}>
