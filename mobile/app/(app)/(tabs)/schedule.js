      ...
      <View className="px-5 pt-2">
        ...
        <View className="flex-row mb-4 bg-dark-card rounded-xl p-1">
          {[
            ...
            { key: "availability", label: "Trainer Hours" },
          ].map((t) => (
            <TouchableOpacity
              key={t.key}
      ...
      {tab === "availability" && (
        <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 100 }}>
          {!trainerId && !isTrainer ? (
            ...
            <EmptyState icon="time-outline" title="No availability set" message={
              isTrainer ? "Set your availability in the trainer schedule screen" : "Your trainer hasn't set availability yet"
            } />
          ) : (
