      ...
      <PullToRefresh onRefresh={loadEvents}>
        <View className="px-5 pb-8">
          ...
          <View className="flex-row items-center justify-between mt-2 mb-4">
            ...
            <TouchableOpacity onPress={() => { setCurrentDate(new Date()); setSelectedDate(today); }}>
          ...
          <View className="flex-row items-center justify-between mb-3">
            ...
            <Text className="text-white text-lg font-semibold">{MONTHS[month]} {year}</Text>
            <TouchableOpacity onPress={() => navigate(1)} className="p-2">
                  ...
                  {dayEvents.length > 0 && (
                    <View className="flex-row gap-0.5 mt-0.5">
                      {dayEvents.slice(0, 3).map((e, j) => (
                        <View key={j} style={{ width: 4, height: 4, borderRadius: 2, backgroundColor: EVENT_COLORS[e.type]?.color || "#6B7280" }} />
                      ))}
                    </View>
                  )}
                </TouchableOpacity>
                    ...
                    <View className="flex-row items-center">
                      ...
                      <View className="flex-1">
                        ...
                        <View className="flex-row items-center mt-1 gap-2">
                          <Text style={{ color: style.color }} className="text-xs">{style.label}</Text>
                          {e.datetime && <Text className="text-gray-500 text-xs">{formatTime(e.datetime)}</Text>}
                          {e.status && <Badge text={e.status} color={e.status === "completed" ? "green" : e.status === "scheduled" ? "blue" : "gray"} />}
                        </View>
                      </View>
                    </View>
                  </Card>
