...
// Auto-seed reference data (exercises, foods) — inserts missing entries on every startup
(function autoSeed() {
  ...
  const photoMap = require("../prisma/data/exercise-photos");
  const beforeCount = db.prepare("SELECT COUNT(*) as c FROM exercises").get().c;
  const ins = db.prepare(
    `INSERT OR IGNORE INTO exercises (name, description, category, muscle_group, secondary_muscles, equipment, instructions, video_url, photo_url, tracking_type)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  );
  db.transaction(() => {
    for (const ex of allExercises) {
      const photo = ex.photo_url || photoMap[ex.name] || null;
      ins.run(ex.name, ex.description, ex.category, ex.muscle_group, ex.secondary_muscles || "[]", ex.equipment, ex.instructions, ex.video_url || null, photo, ex.tracking_type || "reps_weight");
    }
  })();
  const afterCount = db.prepare("SELECT COUNT(*) as c FROM exercises").get().c;
  const added = afterCount - beforeCount;
  if (added > 0) {
    console.log(`  -> Auto-seeded ${added} new exercises (total: ${afterCount}).`);
  }

  // Auto-populate photo_url for exercises missing images
  // Uses pre-computed mappings to free-exercise-db (public domain)
  ...
  if (missingPhotos.length > 0) {
    ...
    db.transaction(() => {
      for (const ex of missingPhotos) {
        const url = photoMap[ex.name] || null;
        if (url) update.run(url, ex.id);
      }
    })();
